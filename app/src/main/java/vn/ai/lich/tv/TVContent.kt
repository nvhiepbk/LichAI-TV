...
data class TVEvent(val title:String,val category:String,val summary:String,val meaning:String,val icon:String)
data class TVPrayer(val title:String,val context:String,val body:String)
data class TVRitual(val title:String,val summary:String,val prayers:List<TVPrayer>)
...
object TVContent {
    ...
    fun events(ctx:Context,date:LocalDate,lunar:LunarDate):List<TVEvent>{
        ...
        for(i in 0 until a.length()){
            val e=a.getJSONObject(i); val cal=e.optString("calendar")
            val d=e.optInt("day",-1); val m=e.optInt("month",-1)
            val hit=if(cal=="lunar") d==lunar.day&&m==lunar.month else d==date.dayOfMonth&&m==date.monthValue
            if(hit) out+=TVEvent(e.optString("title"),e.optString("category"),e.optString("summary",e.optString("description")),e.optString("meaning"),e.optString("icon"))
        }
        return out
    }
    fun rituals(ctx:Context,date:LocalDate,lunar:LunarDate):List<TVRitual>{
        ...
        for(i in 0 until a.length()){
            ...
            if(hit){
                ...
                raw+=r to TVRitual(r.optString("title"),r.optString("summary"),ps)
            }
        }
        val superseded= mutableSetOf<String>()
        raw.forEach{(r,_)-> val s=r.optJSONArray("supersedes")?:JSONArray(); for(i in 0 until s.length()) superseded+=s.getString(i)}
    ...
    private fun render(body:String,date:LocalDate,l:LunarDate):String{
        val day=if(l.day==1)"mồng Một" else if(l.day==15)"Rằm" else "ngày ${l.day}"
        val months=arrayOf("","Giêng","Hai","Ba","Tư","Năm","Sáu","Bảy","Tám","Chín","Mười","Mười Một","Chạp")
        val month=months.getOrElse(l.month){l.month.toString()}+(if(l.leap)" nhuận" else "")
        return body.replace("{{lunar_day}}",l.day.toString()).replace("{{lunar_month}}",l.month.toString()).replace("{{lunar_year}}",l.year.toString())
            .replace("{{lunar_day_text}}",day).replace("{{lunar_month_text}}",month).replace("{{lunar_year_can_chi}}",VietnameseLunar.yearCanChi(l.year))
            .replace("{{lunar_date_full}}","ngày $day tháng $month năm ${VietnameseLunar.yearCanChi(l.year)}")
            .replace("{{solar_date_full}}","%02d/%02d/%04d".format(date.dayOfMonth,date.monthValue,date.year))
            .replace("{{worshipper_name}}","................................").replace("{{address}}","................................")
    }
}
